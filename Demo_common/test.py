...
# 批量读取图片用模型进行推导
# read batch of .jpg image to predict
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        test_ids = [line.split() for line in open(FLAGS.test_list)]
        tot = len(test_ids)
        results = list()
        with tf.Session(config=config) as sess:
            ...
            for idx in range(0, tot, batch_size):
                ...
                for i in range(idx, idx_end):
                    image_id = test_ids[i][0]
                    test_path = os.path.join(FLAGS.image_dir, image_id)
                ...
                for i in range(idx, idx_end):
                    print('Iter:{}; Image:{}; Top5_index:{}'.format(str(i), test_ids[i][0],predictions[i - idx].tolist()))
